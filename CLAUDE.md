...
- **RG-08** Seuil d'aiguillage : après validation DA, état ≤ 100 000 XAF clôturé et transmis ; > 100 000 XAF envoyé au DR. Seuil lu dans `parametre_systeme`, jamais codé en dur. **La comparaison n'existe qu'à un seul endroit du module** : `AiguillageService.aiguiller` (service Workflow). Ni la machine à états, ni le service de validation, ni le contrôleur n'en refont une. Voir `docs/decisions/2026-09-01-seuil-aiguillage-et-borne.md`.
...
- **Workflow (6)** : `/processus` (POST), `/processus/{id}`, `/processus/{id}/etat`, `/processus/{id}/soumission`, `/processus/{id}/validation`, `/processus/{id}/retour`. **Les cinq premiers sont livrés** (Sprints 4.1 à 4.3) ; `/retour` relève du Sprint 4.4, qui ouvrira aussi `/validation` au rôle `DIRECTEUR_RESEAU_DR` (au 4.3, il est réservé à `CHEF_UNITE_DA`). Codes d'erreur ajoutés au contrat : `PROCESSUS_EXISTANT` (409) et `FONCTIONNALITE_NON_OUVERTE` (422) au Sprint 4.1 ; `ETAT_INCOMPLET` (422), `PIECE_JOINTE_EXISTANTE` (409) et `DOCUMENT_NON_PRODUIT` (500) au Sprint 4.2 ; `SEUIL_INDISPONIBLE` (500) au Sprint 4.3.
...
- **Poser `transmis_comptabilite` à la clôture** : c'est le Sprint 5 qui le pose, à la publication effective (RG-13).
