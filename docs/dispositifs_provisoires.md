...
| T-02 | Numéro de compte courant sans contrôle de longueur ni de format (11 chiffres attendus) | Équipe | 2026-09-09 | `@Size(max = 20)` seul ; 44 des 45 bénéficiaires en base sont faux | Une faute de frappe crée un bénéficiaire fantôme **et** crédite un mauvais compte, sans erreur visible. **Accompagnement tranché le 2026-09-09** : contrôle sur les nouvelles saisies seulement (la base de production démarrera vide), les 44 lignes de développement ne sont pas corrigées ; le défaut durable est l'exemple faux du contrat d'API §7.1 et le `VARCHAR(30)` du dictionnaire. Correction `@Pattern` à poser avant 7F.4 | En attente |
